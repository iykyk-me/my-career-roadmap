...
const categories: { value: Category; label: string; color: string }[] = [
    { value: 'study', label: '학습', color: 'bg-primary/10 text-primary dark:bg-blue-900/30 dark:text-blue-300' },
    { value: 'project', label: '프로젝트', color: 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-300' },
    { value: 'certificate', label: '자격증', color: 'bg-sub/10 text-sub dark:bg-purple-900/30 dark:text-purple-300' },
    { value: 'activity', label: '활동', color: 'bg-point/10 text-point dark:bg-orange-900/30 dark:text-orange-300' },
...
export default function GoalChecklist({ goals, onAdd, onToggle, onDelete }: GoalChecklistProps) {
    ...
    return (
        <div className="bg-white dark:bg-slate-900 rounded-xl shadow-sm border border-slate-200 dark:border-slate-800 p-6">
            <h3 className="text-lg font-bold mb-4 text-neutral dark:text-slate-100 flex items-center gap-2">
                오늘의 목표
                        ...
                        >
                            ...
                            <div className="flex-1 min-w-0">
                                <p className={cn(
                                    ...
                                    goal.completed ? "text-slate-400 line-through" : "text-neutral dark:text-slate-100"
                                )}>
                                    {goal.text}
                                </p>
                            </div>
            ...
            <form onSubmit={handleSubmit} className="flex gap-2">
                <select
                    ...
                    className="w-24 px-2 py-2 text-sm border rounded-lg dark:bg-slate-800 dark:border-slate-700 dark:text-white focus:outline-none focus:ring-2 focus:ring-primary"
                >
                ...
                <input
                    ...
                    className="flex-1 px-3 py-2 text-sm border rounded-lg dark:bg-slate-800 dark:border-slate-700 dark:text-white focus:outline-none focus:ring-2 focus:ring-primary"
                />
                <button
                    type="submit"
                    className="p-2 bg-primary text-white rounded-lg hover:bg-teal-700 transition-colors"
                >
